...
{
    public static class ModelGenerateHelper
    {
        /// <summary>
        /// 获取数据表中所有列名
        /// </summary>
        ...
        public static string GetUserSealedCode(CodeGenerate m, string Code)
        {
            ...
            sb.Append(ModelGenerateHelper.GetSpace(1) + "public sealed class " + m.ClassName);
            ModelGenerateHelper.NewLine(sb);
            sb.Append(ModelGenerateHelper.SetBrace(Code, 1));
            ModelGenerateHelper.NewLine(sb);
            return sb.ToString();
        }
